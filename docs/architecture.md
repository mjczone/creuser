...
- Matrix views + KPI APIs (post-v0.1) — declarative slicings of the entity projection, bring-your-own charting via workspace `<head>` script registration. Design doc shipped (`docs/wip/matrix-views-design.md`); implementation parked until real dashboards drive concrete consumers. Decisions locked in: structured query DSL (not free-form SQL), L + C shapes for v0.1.x, YAML-on-disk (no `cr.matrix_views` table), `cr.matrix_view_cache` invalidation piggy-backs on `projection-sync` convention-versions hash. Bring-your-own charting via workspace `<head>` script registration.
- ~~Dashboard composer Phase 1 (scaffold + backend + dockview shell) shipped.~~ `cr.dashboard_groups` + `cr.dashboards` tables; `IDashboardStore` + `dashboardsRepository`; `DashboardSeeder` populates Home + Operations group on workspace create; `DashboardsEndpoints` for CRUD + nav-tree. SPA: `dockview-vue` v6, `WidgetRegistry` + `WidgetHost` + `DashboardPage` with layout serialize/deserialize, `useDashboardsStore` Pinia cache, `MainLayout` icon-bar driven by the dashboards nav-tree. Eight v1 widget types register against a single `PlaceholderWidget` to prove the loop end-to-end; real widget implementations land one-per-commit in Phase 2. Implementation contract: `docs/wip/dashboard-composer-design.md`.
