...
public sealed class DbInitializer : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        await using (var conn = await _ds.OpenConnectionAsync(cancellationToken))
        {
            ...
            await conn.CreateTableIfNotExistsAsync<plugins>(
                tx: null,
                cancellationToken: cancellationToken
            );
            await conn.CreateTableIfNotExistsAsync<workspace_plugins>(
                tx: null,
                cancellationToken: cancellationToken
            );
            await conn.CreateTableIfNotExistsAsync<workspace_plugin_settings>(
                tx: null,
                cancellationToken: cancellationToken
            );
            ...
            await conn.CreateTableIfNotExistsAsync<dashboards>(
                tx: null,
                cancellationToken: cancellationToken
            );
