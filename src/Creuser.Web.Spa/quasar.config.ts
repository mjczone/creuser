...
export default defineConfig((ctx) => {
  return {
    ...
    boot: ['i18n', 'branding', 'auth', 'monaco', 'widgets'],
    ...
    build: {
      ...
      // Build SPA output directly into the .NET project's wwwroot so
      // `dotnet publish` bundles it. ASP.NET Core serves index.html and
      // assets from there in production; in dev the SPA dev server is the
      // origin and proxies /api, /hub, /scalar to ASP.NET (see devServer).
      distDir: '../Creuser.Web/wwwroot',
    ...
    devServer: {
      ...
      proxy: {
        // Proxy REST + Scalar/OpenAPI to the .NET backend (see launchSettings.json).
        '/api': {
        ...
        // Proxy SignalR with WebSocket upgrade.
        '/hub': {
          target: 'http://localhost:5128',
          changeOrigin: true,
          ws: true,
